...
// Define the structure for our entry items
interface Entry {
  inputText: string;
  starDate: string;
  endDate: string;
...
// Use proper types for reactive entries and arrays
...
// Use typed arrays for storing entries
// let myDitailsArray = ref<Entry[]>([]);
// let educationArray = ref<Entry[]>([]);
// let workArray = ref<Entry[]>([]);
...
// Skills handling with proper primitive types
let skill = ref<string>('');


// Generic function to add items based on type
function addToArr(dataType: 'skills' | 'myDitails' | 'education' | 'work'): void {
  if (dataType === 'skills') {
   ...
   //myDitailsArray.value.push({ ...myDitails });
    ...
    //educationArray.value.push({ ...education });
...
// Generic function to remove items by index from the appropriate collection
function removeSkill(index: number, type: 'skills' | 'myDitails' | 'education' | 'work'): void {
  if (type === 'skills') {
  ...
  <mp-modal modalId="code-style-edit-modal">
    ...
    <template #modal-body>
      ...
      <div class="row-element">
        ...
        <div class="w-50">
          ...
          <div class="input-with-btn">
            <input v-model="myDitails.value" @keydown.enter="addToArr('myDitails')" type="text" id="my-ditails-value" placeholder="Enter value" />
          ...
          <div class="tags-container">
            <div class="tag-item" v-for="(ditail, index) in myDitailsArray" :key="index">
              ...
              <font-awesome-icon @click="removeSkill(index, 'myDitails')" icon="fa-solid fa-x" />
            </div>
          </div>
        </div>
      </div>

      <!-- Education Section (example structure) -->
      <div class="row-element">
        <div class="w-50">
        ...
        <div class="w-50">
          <div class="date-picker-inputs">
            ...
            <datepicker v-if="!education.tillNow" :minimum-view="'month'" :maximum-view="'month'" v-model="education.endDate"></datepicker>
          </div>
          ...
          <div class="input-with-btn">
            <input v-model="education.inputText" @keydown.enter="addToArr('education')" type="text" id="education" placeholder="Enter education details" />
          ...
          <div class="tags-container">
            <div class="tag-item" v-for="(edu, index) in educationArray" :key="index">
              ...
              <font-awesome-icon @click="removeSkill(index, 'education')" icon="fa-solid fa-x" />
            </div>
          </div>
        </div>
      </div>

      <!-- Work Section (example structure) -->
      <div class="row-element">
        <div class="w-50">
        ...
        <div class="w-50">
          ...
          <div class="input-with-btn">
            <input v-model="work.inputText" @keydown.enter="addToArr('work')" type="text" id="work" placeholder="Enter work details" />
          ...
          <div class="tags-container">
            <div class="tag-item" v-for="(w, index) in workArray" :key="index">
              ...
              <font-awesome-icon @click="removeSkill(index, 'work')" icon="fa-solid fa-x" />
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="row-element">
        ...
        <div class="w-50">
          <div class="input-with-btn">
            <input v-model="skill" @keydown.enter="addToArr('skills')" type="text" id="my-skils" placeholder="Enter skills" />
            <button @click="addToArr('skills')">Add</button>
          </div>
          <div class="tags-container">
            <div class="tag-item" v-for="(s, index) in skills" :key="index">
              <span class="tag-text">{{ s }}</span>
              <font-awesome-icon @click="removeSkill(index, 'skills')" icon="fa-solid fa-x" />
            </div>
          </div>
        </div>
      </div>
